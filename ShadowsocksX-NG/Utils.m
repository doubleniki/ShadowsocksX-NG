...
void ScanQRCodeOnScreen(void) {
    ...
    if (@available(macOS 10.15, *)) {
        ...
        os_log_info(OS_LOG_DEFAULT, "Screen Recording Permission Status: %{public}@", hasPermission ? @"Granted" : @"Not Granted");

        if (!hasPermission) {
            os_log_info(OS_LOG_DEFAULT, "Requesting Screen Recording Permission...");
            CGRequestScreenCaptureAccess();

            /* check permission status after request */
            hasPermission = CGPreflightScreenCaptureAccess();
            os_log_info(OS_LOG_DEFAULT, "Screen Recording Permission Status After Request: %{public}@", hasPermission ? @"Granted" : @"Not Granted");

            if (!hasPermission) {
                os_log(OS_LOG_DEFAULT, "Screen Recording Permission Denied");
        ...
        os_log_info(OS_LOG_DEFAULT, "Proceeding with screen capture...");
    }
    ...
    os_log_info(OS_LOG_DEFAULT, "Found %d displays", dspCount);
    ...
    for (unsigned int displaysIndex = 0; displaysIndex < dspCount; displaysIndex++) {
        ...
        for (CIQRCodeFeature *feature in features) {
            os_log_info(OS_LOG_DEFAULT, "Found QR Code: %{public}@", feature.messageString);
